...
import { hasVaultRecoveryPhrase } from "@/lib/vault/recovery-phrase-store";
...
export async function getWorkspaceData() {
  ...
  const [projects, secrets, files, folders, conversations, activity, subscription, authenticators, recovery] = await Promise.all([
    ...
    supabase.from("subscriptions").select("product_key,status,current_period_end,storage_quota_bytes,entitlement_pro,cloud_plan,paddle_customer_id,scheduled_change_action,scheduled_change_effective_at").eq("workspace_id", context.workspaceId).order("updated_at", { ascending: false }),
    supabase.from("authenticator_entries").select("id,issuer,account_name,notes,pinned,last_used_at,created_at,updated_at").eq("workspace_id", context.workspaceId).eq("owner_id", context.userId).order("issuer"),
  ...
  if (authenticators.error) {
    const fallback = await supabase.from("authenticator_entries").select("id,issuer,account_name,pinned,last_used_at,created_at,updated_at").eq("workspace_id", context.workspaceId).eq("owner_id", context.userId).order("issuer");
    if (!fallback.error) {
      authenticatorRows = (fallback.data ?? []).map((row) => ({ ...row, notes: null }));
    } else {
      const legacy = await supabase.from("authenticator_entries").select("id,issuer,account_name,created_at,updated_at").eq("workspace_id", context.workspaceId).eq("owner_id", context.userId).order("issuer");
      authenticatorRows = (legacy.data ?? []).map((row) => ({ ...row, notes: null, pinned: false, last_used_at: null }));
    }
  }
  const recoveryPhraseConfigured = await hasVaultRecoveryPhrase(context.userId);
  return { context, projects: projects.data ?? [], secrets: secrets.data ?? [], files: files.data ?? [], folders:folders.data??[], conversations: conversations.data ?? [], activity: activity.data ?? [], subscriptions: subscription.data ?? [], authenticators: authenticatorRows, recovery: recovery.data ?? [], recoveryPhraseConfigured };
}

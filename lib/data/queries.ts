...
export async function getWorkspaceData() {
  ...
  const [projects, secrets, files, folders, conversations, activity, subscription, authenticators, recovery] = await Promise.all([
    ...
    supabase.from("cloud_files").select("id,original_filename,mime_type,size_bytes,status,project_id,folder_id,deleted_at,created_at,updated_at,projects(name)").eq("workspace_id", context.workspaceId).is("deleted_at", null).order("updated_at", { ascending: false }),
